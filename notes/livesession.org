...
- Output :: If possible a monotype τ such that Γ ⊢ e : Closure(Γ, τ) is derivable by the type rules
  and if Γ ⊢ e : σ is some other type scheme which is provable then σ ≤ Closure
...
Suppose under Γ the most general type scheme that you can deduce for e is σ = ∀ α₁ ... ∀ αₙ . τ
...
Take all the variables that are free in τ but not in Γ and do generalisation some many times.
That will give you the extended generalisation rule.
